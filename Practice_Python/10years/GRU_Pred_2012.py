# Predict stock price of Dow Jones using GRU for 2012
import json
import numpy as np
import pandas as pd
...
for i in range(len(x_t)):
    ...
    gru_model.fit(x, y, epochs=200, batch_size=128, validation_split=0.2)

    model_input = x_t[i].reshape(1, window_size, len(variable))
    gru_pred = np.append(gru_pred, gru_model.predict(model_input)[0])
...
gru_pred = scaler_y.inverse_transform(gru_pred.reshape(len(x_t),1))
with open(file='../../../DeepLearning_result_data/gru_pred_2012.pkl', mode='wb') as f:
    pickle.dump(gru_pred, f)